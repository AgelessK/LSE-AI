...
Orosz, G. (2026, January 6). When AI writes almost all code, what happens to software engineering? *The Pragmatic Engineer* newsletter. Opinion/synthesis deepdive on the late-2025 capability inflection (Gemini 3, Opus 4.5, GPT-5.2). Cited here for the practitioner refrain that "writing code fast was never the bottleneck," which this paper reads as naming the two-loop bottleneck — verification (Loop 1) and alignment (Loop 2). (The Atlassian and Cortex figures it also discusses are cited directly above against their primary reports.) A single-author analysis piece, not a study; exact article URL to be confirmed. Distinct from Orosz & Nilsson (2026), the survey-trends articles.
